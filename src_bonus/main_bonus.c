...
void	show_commands(t_fdf *map)
{
    ...
    mlx_string_put (map->p_mlx, map->p_win, 20, 150,
		0xFFFFFF, "E - Increase zoom");
	mlx_string_put (map->p_mlx, map->p_win, 20, 170,
		0xFFFFFF, "Q - Decrease zoom");
	mlx_string_put (map->p_mlx, map->p_win, 20, 190,
		0xFFFFFF, "< - Left rotation");
	mlx_string_put (map->p_mlx, map->p_win, 20, 210,
		0xFFFFFF, "> - Right rotation");
	if (map->proj_id == false)
		mlx_string_put (map->p_mlx, map->p_win, 20, 230,
...
int command_pressed(int key, t_fdf *map)
{
    ...
    if (key == 102)  // 'F' key - decrease depth
        map->hor_view -= 0.01;
    if (key == 101)  // 'E' key - increase zoom
        map->zoom += 0.5;
    if (key == 113)  // 'Q' key - decrease zoom
        map->zoom -= 0.5;
    if (key == 65361)  // Left arrow key - left rotation
        map->rotat_id -= 0.1;
    if (key == 65363)  // Right arrow key - right rotation
        map->rotat_id += 0.1;
    if (key == 65289)  // Tab key - switch projection mode
        map->proj_id = !map->proj_id;
    draw_map(map);
		return(0);
}
